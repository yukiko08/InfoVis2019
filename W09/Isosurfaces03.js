function Isosurfaces( volume, isovalue, screen )
{
    ...
    //Clamp:function(a,b,d){return Math.max(b,Math.min(d,a))}
        ...
        for ( var y = 0; y < volume.resolution.y - 1; y++ )
        {
            for ( var x = 0; x < volume.resolution.x - 1; x++ )
            {
                ...
                for ( var j = 0; lut.edgeID[index][j] != -1; j += 3 )
                {
                    ...
                    var C0 = new THREE.Color().setHex( cmap[ 255 ][1] );
                    var C1 = new THREE.Color().setHex( cmap[ 255 ][1] );
                    var C2 = new THREE.Color().setHex( cmap[ 255 ][1] );
    ...
    function cell_node_indices( cell_index )
    {
        ...
        var id0 = cell_index; //0
        var id1 = id0 + 1;  //1
        var id2 = id1 + lines; //121
        var id3 = id0 + lines; //120
        var id4 = id0 + slices; //120*120
        var id5 = id1 + slices; //120*120+1
        var id6 = id2 + slices; //120*120+121
        var id7 = id3 + slices; //120*120+120
    ...
    function interpolated_vertex( v0, v1, s )
    {
      ...
      var ind0 = v0.x + v0.y*lines + v0.z*slices;
      var ind1 = v1.x + v1.y*lines + v1.z*slices;

      var s0 = volume.values[ind0][0];
      var s1 = volume.values[ind1][0];
